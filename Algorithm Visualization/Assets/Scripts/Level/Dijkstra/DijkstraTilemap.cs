...
public class DijkstraTilemap : MonoBehaviour
{
    ...
    /// Dijkstra算法可视化
    /// </summary>
    public void Dijstra_Visualization()
    {
        if (currentStatus == Status.Busy)
        {
            if (isPrinted == false)
            {
                isPrinted = true;
                walkedPath = dijkstra.GetWalkedPath(map, new DijkstraPoint(begPos.Row, begPos.Col), new DijkstraPoint(endPos.Row, endPos.Col));
                path = dijkstra.GetDijkstraPath(map, new DijkstraPoint(begPos.Row, begPos.Col), new DijkstraPoint(endPos.Row, endPos.Col), out isFinded);
